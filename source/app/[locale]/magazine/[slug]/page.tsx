      ...
      <div className="mx-auto max-w-[44rem] px-6 py-14 lg:px-8 lg:py-20">
        ...
        {p.sections.map((s, i) => (
          <section key={i} className="mt-12 first:mt-10">
            ...
            {/* tvb-section-image — 섹션에 image 가 있을 때만 렌더 */}
            {"image" in s && (s as { image?: string }).image ? (
              <figure className="mt-8">
                ...
                />
                {(s as { imageCaption?: string }).imageCaption ? (
                  <figcaption className="mt-3 text-center text-xs tracking-wide text-ink-soft/70">
                    {(s as { imageCaption?: string }).imageCaption}
                  </figcaption>
                ) : null}
              </figure>
